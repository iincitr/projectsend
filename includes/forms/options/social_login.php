...
// Remove any blocks that don't apply (like LDAP which now has its own page)
unset($blocks['ldap']);

foreach ($blocks as $block_info) {
    $block_title = $block_info['title'];
    $description = isset($block_info['description']) ? $block_info['description'] : '';
    ?>
    ...
    <?php
    foreach ($block_info['items'] as $item) {
...
<div class="options_column">
    <h5><?php _e('Additional Settings','cftp_admin'); ?></h5>
    <p><?php _e('Configure general behavior for social login users.','cftp_admin'); ?></p>
